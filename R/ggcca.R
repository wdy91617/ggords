#' @title \code{"CCA"} ordination plot
#' @description
#' Output cca ordination plot produced by \code{\link[vegan]{cca}}.
#'
#' @details Control of some parts is put in the function. Theme of figure can be set by theme of ggplot2.
#' 
#' @param ord An object produced by \code{\link[vegan]{cca}}.
...
#' # Set a theme 
#' require(ggplot2)
#' ggcca(Spe.cca, group = grl, fcol = "white", facol = "white") + theme_dark()
#'
#' # Remove the arrow
...
ggcca <- function(ord, groups = NULL, axes = c(1,2), display = "bp", scaling = 2, obslab = FALSE, moblabs = NULL,
...
# Extracting the proportion explained
	...
	xylab <- paste0("CCA", axes, " (", round(100 * expvar, 2), "%)")

#############################################################################################################################
# plot
	p <- ggplot(obs, aes_string(x = 'CCA1', y = 'CCA2')) +
...
# Add col variables
	...
	##########################################################
		if(!is.null(sprotate)){
		  angles <- with(spes, (180/pi) * atan(CCA2 / CCA1))
		  hjusts <- with(spes, (1 - sprotate * sign(CCA1)) / 2)
		} else {
	...
	##########################################################
		...
		if(!is.null(spearrow)){
		  p <- p + geom_segment(data = spes[c("CCA1","CCA2")]*0.9, aes_string(x = 0, y = 0, xend = 'CCA1', yend = 'CCA2'),
	...
	##########################################################
		if(spelab){
			if(!is.null(spmapsize)){
				splong <- sqrt((spes$CCA1)^2+(spes$CCA2)^2) + spmapsize
				p <- p + geom_text(data = spes, aes_string(x = 'CCA1', y = 'CCA2', label = "splabs", size = "splong", angle = "angles", hjust = "hjusts"), 
					 parse = TRUE, col = specol, family = speFonts, fontface = speface) + guides(size=FALSE)
			} else {
				p <- p + geom_text(data = spes, aes_string(x = 'CCA1', y = 'CCA2', label = "splabs", angle = "angles", hjust = "hjusts"), parse = TRUE,
					 size = spesize, col = specol, family = speFonts, fontface = speface)
			}
		} else {
		  p <- p + geom_point(data = spes, aes_string(x = 'CCA1', y = 'CCA2'), size = spesize, col = specol, shape = spepch)
		}
	}
...
# Add fitting variables, display == "bp"
	if (display == "bp") {
		...
		##########################################################
		if(!is.null(frotate)){
		  fangles <- with(fits, (180/pi) * atan(CCA2 / CCA1))
		  fhjusts <- with(fits, (1 - frotate * sign(CCA1)) / 2)
		} else {
		...
		##########################################################
		if(!is.null(farrow))
			p <- p + geom_segment(data = fits[c("CCA1","CCA2")]*0.9, aes_string(x = 0, y = 0, xend = 'CCA1', yend = 'CCA2'),
				 linetype = faline, size = falwd, col = facol,arrow = arrow(length = unit(farrow, "cm"),type = "closed"))
		##########################################################
		if(!is.null(fmapsize)){
			flong <- sqrt((fits$CCA1)^2+(fits$CCA2)^2) + fmapsize
			p <- p + geom_text(data = fits, aes_string(x = 'CCA1', y = 'CCA2', label = "flabs", size = "flong", angle = "fangles", hjust = "fhjusts"),
				 parse = TRUE, col = fcol, family = fFonts, fontface = fface)+ guides(size=FALSE)
		} else {
		  p <- p + geom_text(data = fits, aes_string(x = 'CCA1', y = 'CCA2', label = "flabs", angle = "fangles", hjust = "fhjusts"), parse=  TRUE,
...
# confidence ellipses
	if(!is.null(groups) && ellipse) {
		...
		ellxy <- ddply(obs, 'groups', function(x) {
			  ...
			  sigma <- var(cbind(x$CCA1, x$CCA2))
			  mu <- c(mean(x$CCA1), mean(x$CCA2))
			  ed <- sqrt(qchisq(ellprob, df = 2))
			  data.frame(sweep(circle %*% chol(sigma) * ed, 2, mu, FUN = '+'), groups = x$groups[1])
			  })
		names(ellxy)[1:2] <- c('CCA1', 'CCA2')
		p <- p + geom_path(data = ellxy, aes_string(x = 'CCA1', y = 'CCA2', colour = "groups" ), size = cirlwd, linetype = cirline)  
	}
