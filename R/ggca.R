#' @title \code{"CA"} ordination plot
#' @description
#' Output ca ordination plot produced by \code{\link[vegan]{cca}}.
#'
...
ggca <- function(ord,groups = NULL, axes = c(1,2), scaling = 2, obslab = FALSE, moblabs = NULL,
...
# Extracting the proportion explained
	...
	xylab <- paste0("CA", axes, " (", round(100 * expvar, 2), "%)")

#############################################################################################################################
# plot
	p <- ggplot(obs, aes_string(x = 'CA1', y = 'CA2')) +
...
# Add col variables
	...
	##########################################################
		if(!is.null(sprotate)){
		  angles <- with(spes, (180/pi) * atan(CA2 / CA1))
		  hjusts <- with(spes, (1 - sprotate * sign(CA1)) / 2)
		} else {
	...
	##########################################################
		...
		if(!is.null(spearrow)){
		  p <- p + geom_segment(data = spes[c("CA1","CA2")]*0.9, aes_string(x = 0, y = 0, xend = 'CA1', yend = 'CA2'),
	...
	##########################################################
		if(spelab){
			if(!is.null(spmapsize)){
				splong <- sqrt((spes$CA1)^2+(spes$CA2)^2) + spmapsize
				p <- p + geom_text(data = spes, aes_string(x = 'CA1', y = 'CA2', label = "splabs", size = "splong", angle = "angles", hjust = "hjusts"), 
					 parse = TRUE, col = specol, family = speFonts, fontface = speface) + guides(size=FALSE)
			} else {
				p <- p + geom_text(data = spes, aes_string(x = 'CA1', y = 'CA2', label = "splabs", angle = "angles", hjust = "hjusts"), parse = TRUE,
					 size = spesize, col = specol, family = speFonts, fontface = speface)
			}
		} else {
		  p <- p + geom_point(data = spes, aes_string(x = 'CA1', y = 'CA2'), size = spesize, col = specol, shape = spepch)
		}
	}
...
# Add fitting variables
	if(!is.null(envs)){
		...
		##########################################################
		if(!is.null(frotate)){
		  fangles <- with(fits, (180/pi) * atan(CA2/CA1))
		  fhjusts <- with(fits, (1 - frotate * sign(CA1)) / 2)
		} else {
		...
		##########################################################
		if(!is.null(farrow))
			p <- p + geom_segment(data = fits[c("CA1","CA2")]*0.9, aes_string(x = 0, y = 0, xend = 'CA1', yend = 'CA2'),
				 linetype = faline, size = falwd, col = facol,arrow = arrow(length = unit(farrow, "cm"),type = "closed"))
		##########################################################
		if(!is.null(fmapsize)){
			flong <- sqrt((fits$CA1)^2+(fits$CA2)^2) + fmapsize
			p <- p + geom_text(data = fits, aes_string(x = 'CA1', y = 'CA2', label = "flabs", size = "flong", angle = "fangles", hjust = "fhjusts"),
				 parse = TRUE, col = fcol, family = fFonts, fontface = fface)+ guides(size=FALSE)
		} else {
		  p <- p + geom_text(data = fits, aes_string(x = 'CA1', y = 'CA2', label = "flabs",angle = "fangles", hjust = "fhjusts"), parse=  TRUE,
...
# confidence ellipses
	if(!is.null(groups) && ellipse) {
		...
		ellxy <- ddply(obs, 'groups', function(x) {
			  ...
			  sigma <- var(cbind(x$CA1, x$CA2))
			  mu <- c(mean(x$CA1), mean(x$CA2))
			  ed <- sqrt(qchisq(ellprob, df = 2))
			  data.frame(sweep(circle %*% chol(sigma) * ed, 2, mu, FUN = '+'), groups = x$groups[1])
			  })
		names(ellxy)[1:2] <- c('CA1', 'CA2')
		p <- p + geom_path(data = ellxy, aes_string(x = 'CA1', y = 'CA2', colour = "groups" ), size = cirlwd, linetype = cirline)  
	}
